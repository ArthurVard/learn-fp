...
filter ::
  ...
  -> List a
filter _ Nil = Nil
filter pred (x :. xs) = ifThenElse (pred x) (x :. filter pred xs) (filter pred xs)
...
seqOptional (m :. ms) =  m `bindO` (\x -> seqOptional ms `bindO` (\xs -> Full $ x:.xs))
    where
      bindO = flip bindOptional
--    foldRight bindOptional Empty xs
...
find ::
  ...
  -> Optional a
find _ Nil       = Empty
find p (x :. xs) = ifThenElse (p x) (Full x) (find p xs)
